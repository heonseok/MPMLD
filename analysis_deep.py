...
def collate_reconstructions(dataset, description, model, recon_type_list):
    model_path = os.path.join(
        base_path, dataset, description, model, 'reconstruction')

    fig, axes = plt.subplots(nrows=REPEAT, ncols=len(recon_type_list), figsize=(
        unit_length*len(recon_type_list), unit_length*REPEAT))
    for repeat_idx in range(REPEAT):
        for recon_idx, recon_type in enumerate(recon_type_list):
            ax = axes[repeat_idx][recon_idx]
            ax.imshow(mpimg.imread(os.path.join(
                model_path, 'repeat' + str(repeat_idx), recon_type + '.png')))
...
def collate_disentanglement_result(dataset, description, model):
    ...
    for repeat_idx in range(REPEAT):
        ...
        class_acc_dict = np.load(os.path.join(
            repeat_path, 'class_acc.npy'), allow_pickle=True).item()
        membership_acc_dict = np.load(os.path.join(
            repeat_path, 'membership_acc.npy'), allow_pickle=True).item()
        ...
        for repeat_idx in range(REPEAT):
            if 'raw' in model:
                repeat_path = os.path.join(
                    model_path, 'repeat' + str(repeat_idx), 'acc.npy')
                recon_type = 'raw'
            else:
                repeat_path = os.path.join(
                    model_path, recon_type, 'repeat' + str(repeat_idx), 'acc.npy')
...
def collate_attack_result(dataset, description, model, recon_type_list):
    model_path = os.path.join(
        base_path, dataset, description, model, 'attack', 'ResNet18_lr0.0001_bs32')
    attack_type_list = [
        'black',
        'white',
    ]
    ...
    for recon_idx, recon_type in enumerate(recon_type_list):
        for attack_type in attack_type_list:
            for repeat_idx in range(REPEAT):
                if 'raw' in model:
                    repeat_path = os.path.join(
                        model_path, attack_type, 'repeat' + str(repeat_idx), 'acc.npy')
                    recon_type = 'raw'
                else:
                    repeat_path = os.path.join(
                        model_path, recon_type, attack_type, 'repeat' + str(repeat_idx), 'acc.npy')
                acc_dict = np.load(repeat_path, allow_pickle=True).item()
