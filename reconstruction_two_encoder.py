...
class TwoReconstructor(object):
    def __init__(self, args):
        ...
        if args.dataset in ['MNIST', 'Fashion-MNIST', 'CIFAR-10', 'SVHN']:
            ...
            self.nets['class_encoder'] = module.VAEConvEncoder(int(self.z_dim/2), self.num_channels)
            self.nets['membership_encoder'] = module.VAEConvEncoder(int(self.z_dim/2), self.num_channels)
    ...
    def reconstruct(self, dataset_dict, reconstruction_type_list):
        try:
            ...
            sys.exit(1)
        self.nets['class_encoder'].eval()
        self.nets['membership_encoder'].eval()
